...
int main()
{
   ...
   and returns a new file descriptor referring to that socket. At this point, connection is established between client and server, and they are ready to transfer data. */
    ...
    while (1)
    {
		/*If the readfds argument is not NULL, it points to an object of type fd_set that on input specifies the 
		 * file descriptors to be checked for being ready to read, and on output indicates which file descriptors are 
		 * ready to read.FD_ZERO(&fdset) initializes the file descriptor set fdset to have zero bits for all file descriptors.*/
        ...
        /* loop for all clients and set file descriptor bits for all connected sockets. If a new larger socketid is found, 
         * make it max_sockid */
        {
